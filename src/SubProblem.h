...
public:
   ...
   */
    ...
    */ 
    ...
    bool            only_add_opt_sol;
    bool            allow_frac_cut;
    int             init_cut_mode;
    int             debug_print_mode;
    /**
...
public:
    ...
    // destroy prob, fx, cutval(partial x_i), rhs is rely on the current solution x. 
    double destroy_prob();
    double cal_fx();
    // cutval i equals to [ partial f(x) / partial x_i ]
   ...
   // 之后将该返回值改为return status 确保能够得到问题的求解状态
    ...
    vector<int> cal_optimal_scene_with_target_cons(Scene_SSL& scene_ssl);
   ...
   // Use this to set CPLEX's benders_callback, if ture, add current x's cut
    bool __is_current_cut_should_add();
   ...
   // use this to print current x, cut_ind, cut_val, rhs
    void print_OA_cut_info(); 
    // Use this to print the target index and f(x)
    void print_model();
    // Use this to print the model and temp x
    void print_debug();
    // Use this to print all the parameters in sp
    void print_pramas();
    // Use this to print the cut generated by x
    void print_OA_cut(); 
